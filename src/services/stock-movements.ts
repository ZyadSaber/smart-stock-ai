...
export async function getStockMovementsPageData(
  ...
  if (searchParams.date_from) {
    movementsQuery = movementsQuery.gte("created_at", searchParams.date_from);
  }
  if (searchParams.date_to) {
    const endDate = new Date(searchParams.date_to);
    endDate.setHours(23, 59, 59, 999);
    movementsQuery = movementsQuery.lte("created_at", endDate.toISOString());
  }
  ...
  const flattenedMovements: StockMovement[] = (movements || []).map((m) => ({
    ...m,
    ...
    to_warehouse: Array.isArray(m.to_warehouse)
      ? m.to_warehouse[0]
      : m.to_warehouse,
    created_by_user: Array.isArray(m.created_by_user)
      ? m.created_by_user[0]
      : m.created_by_user,
  }));
